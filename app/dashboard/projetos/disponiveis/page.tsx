  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setApplications(appsRes.data.data); // 👈 importante (seu backend usa data.data)
      } catch (err) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {projects.map((project: any) => {
          ...
          const alreadyApplied = applications.some(
            (app: any) => app.projectId === project.id
          );
          ...
          return (
            ...
            >
              <div>
                ...
                {alreadyApplied && (
                  <span className="inline-block mt-3 px-3 py-1 text-xs bg-green-100 text-green-700 rounded-full font-medium">
                    Já se candidatou
                  </span>
                )}
